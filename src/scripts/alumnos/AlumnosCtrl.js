(function () {
    ...
    angular.module("myvcFrontApp").controller('AlumnosCtrl', [
        ...
        'Acentos',
        '$timeout',
        function ($scope, App, $rootScope, $state, $interval, uiGridConstants, uiGridEditConstants, $modal, $filter, AuthService, toastr, $http, DownloadServ, Upload, Acentos, $timeout) {
            var append3,
                appendPopover,
                appendPopover1,
                appendPopover2,
    ...
    ---------------- ---------------- ---------------- ---------------- ---------------- */
                ...
                if (grupos_ant.length > 0) {
                    ...
                    return $http.put("::matriculas/alumnos-con-grado-anterior", {
                        ...
                        for (k = 0, len2 = ref2.length; k < len2; k++) {
                            ...
                            alumno.fecha_retiro = alumno.fecha_retiro ? new Date(alumno.fecha_retiro.replace(/-/g, '\/')) : alumno.fecha_retiro;
                            alumno.fecha_matricula_ant = alumno.fecha_matricula;
                            alumno.fecha_matricula = alumno.fecha_matricula ? new Date(alumno.fecha_matricula.replace(/-/g, '\/')) : alumno.fecha_matricula;
                            alumno.fecha_nac = alumno.fecha_nac ? new Date(alumno.fecha_nac.replace(/-/g, '\/')) : alumno.fecha_nac;
                            ...
                            for (m = 0, len4 = ref4.length; m < len4; m++) {
                                ...
                                pariente.fecha_nac = pariente.fecha_nac ? new Date(pariente.fecha_nac.replace(/-/g, '\/')) : pariente.fecha_nac;
                            }
                        ...
                        for (o = 0, len6 = ref6.length; o < len6; o++) {
                            ...
                            alumno.fecha_retiro = alumno.fecha_retiro ? new Date(alumno.fecha_retiro.replace(/-/g, '\/')) : alumno.fecha_retiro;
                            alumno.fecha_matricula_ant = alumno.fecha_matricula;
                            results.push(alumno.fecha_matricula = alumno.fecha_matricula ? new Date(alumno.fecha_matricula.replace(/-/g, '\/')) : alumno.fecha_matricula);
                        }
                        return results;
                    });
                };
            ...
            $scope.guardarClaves = function () {
                ...
                return $http.put('::alumnos/cambiar-claves', {
                    ...
                    return toastr.error('Hubo un problema cambiando las claves', 'Error');
                });
            };
            $scope.guardarClavesTodosAlumnos = function () {
                ...
                if (resp) {
                    return $http.put('::cambiar-usuarios/poner-password-todos-alumnos', {clave: $scope.clave_cambiar}).then(function (r) {
                        return toastr.success('Claves cambiadas');
                    }, function (r2) {
                        return toastr.error('Hubo un problema cambiando las claves', 'Error');
                    });
            ...
            $scope.guardarClavesTodosAcudientes = function () {
                ...
                if (resp) {
                    return $http.put('::cambiar-usuarios/poner-password-todos-acudientes', {clave: $scope.clave_cambiar}).then(function (r) {
                        return toastr.success('Claves cambiadas');
                    }, function (r2) {
                        return toastr.error('Hubo un problema cambiando las claves', 'Error');
                    });
            ...
            $scope.PonerDocumentoComoUsernameAlumnos = function () {
                ...
                if (resp) {
                    return $http.put('::cambiar-usuarios/poner-documento-como-username-alumnos').then(function (r) {
                        ...
                        return toastr.error('Hubo un problema cambiando los usuarios', 'Error');
                    });
                }
            };
            ...
            $scope.editar = function (row) { // $state.go('panel.cartera.editar', {alumno_id: row.alumno_id})
            ...
            $scope.agregarAcudiente = function (rowAlum) {
                ...
                return modalInstance.result.then(function (acud) {
                    return rowAlum.subGridOptions.data.splice(rowAlum.subGridOptions.data.length - 1, 0, acud);
                }, function () {});
            };
            // nada
            $scope.cambiarAcudiente = function (rowAlum, acudiente) {
                var modalInstance;
                    ...
                    for (indice = i = 0, len = ref.length; i < len; indice =++ i) {
                        ...
                        if (pariente) {
                            if (pariente.id === acudiente.id) {
                                results.push(rowAlum.subGridOptions.data.splice(indice, 1, acud));
                            } else {
                                results.push(void 0);
                            }
                        } else {
            ...
            // nada
            $scope.quitarAcudiente = function (rowAlum, acudiente) {
                var modalInstance;
                modalInstance = $modal.open({
                    ...
                    for (indice = i = 0, len = ref.length; i < len; indice =++ i) {
                        ...
                        if (pariente) {
                            if (pariente.id === acud.acudiente_id) {
                                results.push(rowAlum.subGridOptions.data.splice(indice, 1));
                            } else {
                                results.push(void 0);
                            }
                        } else {
            ...
            $scope.cambiarCiudad = function (row, tipo_ciudad) {
                var modalInstance;
                modalInstance = $modal.open({
            ...
            $scope.cambiarPazysalvo = function (fila) {
                ...
                return $http.put('::alumnos/guardar-valor', datos).then(function (r) {
            ...
            $scope.crearUsuario = function (row) {
                ...
                if (row.tipo === 'Acudiente') {
                    ...
                    $http.post('::acudientes/crear-usuario', {acudiente: row}).then(function (r) {
                ...
                if (row.tipo !== 'Acudiente') {
                    return toastr.info('Aquí solo puede crear usuario de acudiente', 'Lo sentimos');
                }
            };
            ...
            $scope.cambiaUsernameCheck = function (texto) {
                ...
                return $http.put('::users/usernames-check', {texto: texto}).then(function (r) {
            ...
            $scope.eliminarMatricula = function (row) {
                return $http.delete('::matriculas/destroy/' + row.matricula_id).then(function (r) {
                    ...
                    return toastr.error('No se pudo desmatricular', 'Problema');
                });
            };
            ...
            $scope.religionSelected = function (row, evento) {
            ...
            $scope.tipoDocSeleccionado = function ($item, row) {
                ...
                return $http.put('::' + person + '/guardar-valor', datos).then(function (r) {
                    return toastr.success('Alumno(a) actualizado con éxito', 'Actualizado');
                }, function (r2) {
                    row.tipo_doc = oldValue;
                    return toastr.error('Cambio no guardado', 'Error');
                });
            };
            $scope.matricularUno = function (row, recargar) {
                var datos;
                ...
                return $http.post('::matriculas/matricularuno', datos).then(function (r) {
                    r = r.data;
                    ...
                    return toastr.error('No se pudo matricular el alumno.', 'Error');
                });
            };
            $scope.cambiaEpsCheck = function (texto) {
                $scope.verificandoEps = true;
                return $http.put('::alumnos/eps-check', {texto: texto}).then(function (r) {
            ...
            $scope.reMatricularUno = function (row) {
                ...
                return $http.put('::matriculas/re-matricularuno', datos).then(function (r) {
                    r = r.data;
            ...
            $scope.setAsistente = function (fila) {
                return $http.put('::matriculas/set-asistente', {
                    ...
                    return toastr.error('No se pudo guardar como asistente', 'Error');
                });
            };
            $scope.setPrematriculado = function (fila) {
                return $http.put('::matriculas/prematricular', {
                    ...
                    return toastr.error('No se pudo crear asistente.', 'Error');
                });
            };
            ...
            $scope.cambiarFechaRetiro = function (row) {
                return $http.put('::matriculas/cambiar-fecha-retiro', {
                    ...
                    return toastr.error('No se pudo guardar la fecha', 'Error');
                });
            };
            ...
            $scope.desertar = function (row) {
                ...
                return $http.put('::matriculas/desertar', {
                    ...
                    return toastr.error('No se pudo desertar', 'Problema');
                });
            };
            ...
            $scope.buscar_por = function (campo) {
                ...
                return $http.put('::buscar/por-' + campo, {texto_a_buscar: $scope.texto_a_buscar}).then(function (r) {
                    return $scope.alumnos_encontrados = r.data;
                }, function (r2) {
                    return toastr.error('No se pudo buscar', 'Problema');
                });
            };
            $scope.exportarAlumnos = function () {
                return DownloadServ.download('::simat/alumnos-exportar', 'Alumnos a importar ' + $scope.USER.year + '.xlsx');
            };
            $scope.importarSimat = function (file, errFiles) {
                $scope.f = file;
            ...
            $scope.$on('alumnoguardado', function (data, alum) {
        ...
        'toastr',
        'App',
        function ($scope, $modalInstance, alumno, $http, toastr, App) {
            $scope.alumno = alumno;
            $scope.perfilPath = App.images + 'perfil/';
            $scope.ok = function () {
                $http.delete('::alumnos/destroy/' + alumno.alumno_id).then(function (r) {
                    return toastr.success('Alumno enviado a la papelera.', 'Eliminado');
                }, function (r2) {
                    return toastr.warning('No se pudo enviar a la papelera.', 'Problema');
                });
